...
import com.jubination.backend.report.ReportMessage;
...
public class ReportAPIController {
    ...
    //TESTING
    //@RequestMapping(value="/reportAll/test/{reportId}",method=RequestMethod.GET,headers="Accept=*/*")
    /*public ModelAndView  getAllReportView(HttpServletRequest request,@PathVariable("reportId") String reportId,Principal principal) {
        ModelAndView model=new ModelAndView("reportView");
        List<Report> reportList=new ArrayList<>();
        for(int i=0;i<=15;i++){
        reportList.add(servicePDF.getReport(reportId+i));
        }
    ...
    */
   // @RequestMapping(value="/addAll/test/{reportId}",method=RequestMethod.POST, consumes = MediaType.APPLICATION_JSON_VALUE,headers="Accept=*/*")
   /* public   @ResponseBody MailMessage testReportParsing(@RequestBody MailMessage msg, HttpServletRequest request,@PathVariable("clinic") String clinic,@PathVariable("test") String test,@PathVariable("reportId") String reportId,Principal principal) {
              
            String URL="http://localhost/jubination/pdf/test";
                try{
                    
                    System.out.println("Thyrocare Blood");
                    for(int i=1;i<=15;i++){
                     servicePDF.parsePDFToTextThyrocreBlood(URL+i+".pdf",reportId+i);
                    }
                }
                catch(Exception e){
